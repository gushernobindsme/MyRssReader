...
public class MainActivity extends AppCompatActivity implements SiteListFragment.SiteListFragmentListener,LinkListFragment.LinkListFragmentListener {
     ...
     * ジョブの実行間隔(ms)。
     */
    private static final long INTERVAL = 1L * 60L + 1000L;
    ...
    private void setRepeatingTask(){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP){
            ...
            // API Levelが21未満の場合には、AlarmManagerを使用する
            Intent serviceIntent = new Intent(this,PollingService.class);

            // すでに登録されているかをチェックする
            boolean isExist = (PendingIntent.getService(this,JOB_FETCH_FEED,serviceIntent,PendingIntent.FLAG_NO_CREATE)!= null);

            if (!isExist){
                PendingIntent operation = PendingIntent.getService(this,JOB_FETCH_FEED,serviceIntent,PendingIntent.FLAG_NO_CREATE);
    ...
    public void onSiteDeleted(long id) {
        LinkListFragment fragment = (LinkListFragment) getFragmentManager().findFragmentById(R.id.MasterContainer);
